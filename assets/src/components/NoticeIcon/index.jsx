...
import emptyImage from '@/assets/noticeEmpty.svg';
...
export default class NoticeIcon extends Component {
  ...
  render() {
    const { className, count, popupVisible, bell, mini } = this.props;
    ...
    const trigger = mini ? (
      ...
      <span style={{cursor: 'pointer'}}>
        {NoticeBellIcon}
        {formatMessage({id: 'component.globalHeader.message'})}
        {count && count > 0 && (
          <div style={{width:14, height:14, background:'#F92323', display: 'inline-block', lineHeight:'14px', color: '#fff', fontSize: 12, marginLeft: 4}}>
            {count > 9 ? '9+' : count}
          </div>
        )}
      </span>
